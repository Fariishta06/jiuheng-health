...
async function handleAPI(request, env) {
  ...
  if (pathname === '/api/keys/validate' && method === 'POST') {
    ...
    if (!key.isTest && deviceId) {
      if (!key.deviceId) {
        // 首次使用，绑定设备
        key.deviceId = deviceId;
        key.deviceBoundAt = new Date().toISOString();
        await saveKeys(kv, keys);
      } else if (key.deviceId !== deviceId) {
        // 设备不匹配
        return sendJSON(403, { error: '此密钥已在其他设备激活，每把密钥仅限同一设备使用' });
      }
    }
  ...
  if (pathname === '/api/keys' && method === 'GET') {
    ...
    const result = keys.map(k => ({
      ...
      deviceBound: !k.isTest && !!k.deviceId,
      deviceBoundAt: k.deviceBoundAt || null
    }));
  ...
  /* ── 解除设备绑定（管理员） ── */
  if (pathname === '/api/keys/unbind-device' && method === 'POST') {
    const admins = await getAdmins(kv);
    const auth = await requireAuth(kv, request, admins);
    if (auth.error) return auth.error;
    const admin = auth.admin;
    ...
    key.deviceId = null;
    key.deviceBoundAt = null;
    admin.operations = admin.operations || [];
    admin.operations.unshift({ action: '解除设备绑定', detail: key.key, time: new Date().toISOString() });
    await saveKeys(kv, keys);
    await saveAdmins(kv, admins);
    return sendJSON(200, { ok: true });
  }
